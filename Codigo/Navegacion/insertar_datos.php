...
if (isset($_POST['fecha'], $_POST['calorias'], $_POST['masa_muscular'], $_POST['id_usuario'])) {
    ...
    if (!empty($fecha) && !empty($calorias) && !empty($masa_muscular) && !empty($id_usuario)) {
        ...
        $query = "SELECT * FROM estadisticas WHERE id_usuario = '$id_usuario'";
        $resultado = $conn->query($query);

        if ($resultado->num_rows > 0) {
            // El usuario ya tiene estadísticas registradas, realizar la actualización
            $query = "UPDATE estadisticas SET fecha = '$fecha', calorias_quemadas = '$calorias', masa_muscular = '$masa_muscular' WHERE id_usuario = '$id_usuario'";
        } else {
            // El usuario no tiene estadísticas registradas, realizar la inserción
            $query = "INSERT INTO estadisticas (fecha, calorias_quemadas, masa_muscular, id_usuario) VALUES ('$fecha', '$calorias', '$masa_muscular', '$id_usuario')";
        }
